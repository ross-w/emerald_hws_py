...
def test_mqtt_energy_update_consumption_data(mqtt_client_with_properties):
    ...
    initial_current_hour = initial_consumption["current_hour"]
    initial_last_data_at = initial_consumption["last_data_at"]
...
def test_mqtt_energy_update_handles_missing_consumption_data(
    mqtt_client_with_properties,
):
    """Test that energy updates work when consumption_data is initially missing."""
    import json
    ...
    hws["consumption_data"] = None

    # Process energy update
    client.mqttDecodeUpdate(topic, MQTT_MSG_ENERGY_UPDATE)

    # Verify new consumption structure was created
    consumption = json.loads(hws["consumption_data"])
    assert consumption["current_hour"] == 0.68
    assert consumption["last_data_at"] == "2099-12-31 09:00"
    assert "2099-12-31" in consumption["past_seven_days"]
    assert "2099-12" in consumption["monthly_consumption"]


def test_mqtt_energy_update_handles_invalid_consumption_data(
    mqtt_client_with_properties,
):
    """Test that energy updates work when consumption_data contains invalid JSON."""
    import json
    ...
    # Set invalid JSON in consumption data
    hws["consumption_data"] = "invalid json"
